  ...
  if (!token) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen">
        <h1 className="text-2xl mb-4 text-green-300 font-bold">Login to view your Spotify Playlist</h1>
        <a href={loginUrl}>
...
function UserPlaylists({ token }: { token: string }) {
  ...
  if (!selected) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-900 via-green-900 to-slate-950 flex flex-col items-center p-6">
        <h2 className="text-3xl font-bold mb-8 text-green-300 border-b-4 border-green-500 pb-2 w-full text-center shadow-green-500/40 shadow">Your Spotify Playlists</h2>
          ...
          {playlists.map((pl) => (
            ...
            >
              ...
              <div className="flex-1">
                <div className="font-semibold text-green-200 text-lg">{pl.name}</div>
                <div className="text-sm text-green-400">{pl.owner.display_name}</div>
                <a
  ...
  return <PlaylistSongs token={token} playlistId={selected} onBack={() => setSelected(null)} />;
}

function PlaylistSongs({ token, playlistId, onBack }: { token: string; playlistId: string; onBack: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-900 via-green-900 to-slate-950 flex flex-col items-center p-6">
      <button onClick={onBack} className="mb-6 px-4 py-2 bg-green-700 text-green-100 rounded-xl shadow-green-500/40 shadow border-2 border-green-400 hover:bg-green-800 transition self-start">← Back to Playlists</button>
      <h2 className="text-3xl font-bold mb-8 text-green-300 border-b-4 border-green-500 pb-2 w-full text-center shadow-green-500/40 shadow">Playlist Songs</h2>
      <ul className="w-full max-w-2xl flex flex-col gap-8 items-center">
        {tracks.map(({ track }) => (
          <li
            key={track.id}
